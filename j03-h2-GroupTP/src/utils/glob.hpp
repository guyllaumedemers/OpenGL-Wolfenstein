...
static inline 
std::string translate(std::filesystem::path pattern) {
  std::size_t i = 0, n = pattern.native().size();
  std::string result_string;

  while (i < n) {
    auto c = pattern.native()[i];
    i += 1;
    if (c == '*') {
      result_string += ".*";
    } else if (c == '?') {
      ...
      auto j = i;
      if (j < n && pattern.native()[j] == '!') {
        j += 1;
      }
      if (j < n && pattern.native()[j] == ']') {
        j += 1;
      }
      while (j < n && pattern.native()[j] != ']') {
        j += 1;
      }
      if (j >= n) {
        result_string += "\\[";
      } else {
        auto stuff = std::string(pattern.native().begin() + i, pattern.native().begin() + j);
        if (stuff.find("--") == std::string::npos) {
          ...
          while (true) {
            #ifdef __linux__
                k = pattern.native().find(("-"), k, j);
            #elif _WIN32
                k = pattern.native().find(reinterpret_cast<const wchar_t *>("-"), k, j);
            #endif

            if (k == std::string::npos) {
              break;
            }
            chunks.push_back(std::string(pattern.native().begin() + i, pattern.native().begin() + k));
            i = k + 1;
            k = k + 3;
          }

          chunks.push_back(std::string(pattern.native().begin() + i, pattern.native().begin() + j));
...
static inline 
std::regex compile_pattern(std::filesystem::path pattern) {
  return std::regex(translate(pattern), std::regex::ECMAScript);
}

static inline 
bool fnmatch_case(const fs::path &name, std::filesystem::path pattern) {
...
static inline 
bool has_magic(std::filesystem::path pathname) {
  static const auto magic_check = std::regex("([*?[])");
  std::string path_string{pathname.u8string()};
  return std::regex_search(path_string, magic_check);
}
...
static inline 
bool is_recursive(std::filesystem::path pattern) { return pattern == "**"; }
...
// directory.
static inline 
std::vector<fs::path> glob2(const fs::path &dirname, std::filesystem::path pattern,
                            bool dironly) {
  // std::cout << "In glob2\n";
  std::vector<fs::path> result;
...
static inline 
std::vector<fs::path> glob1(const fs::path &dirname, std::filesystem::path pattern,
                            bool dironly) {
  // std::cout << "In glob1\n";
  auto names = iter_directory(dirname, dironly);
...
static inline 
std::vector<fs::path> glob(std::filesystem::path pathname, bool recursive = false,
                           bool dironly = false) {
  std::vector<fs::path> result;

  auto path = fs::path(pathname);

  if (pathname.native()[0] == '~') {
    // expand tilde
